...
- **Job**: a group of steps executed on a fresh runner. This workflow has a main
  packaging job and a second job that refreshes open pull requests after a
  release.
...
start the workflow because they are outside that path filter.

### Pull request

A pull request event starts a beta build when the PR targets `master` and one of
the listed pipeline/chart paths has changed. Opening the PR, pushing another
commit, or synchronising it with `master` can produce a new PR run.
...
does not restrict the `release` override to `master`, so it must be used with
care.
...
This check currently runs before testing even when `HELM_PUBLISH_ENABLED` is not
`true`, so a valid secret is required for every run of the present workflow.
GitHub masks secret values in logs; the workflow prints only the authenticated
account name and permission result.
...
If there is no staged change, the workflow reports that the chart is already
published. If another run updates the Helm repository during publication, the
step fetches the latest `master`, rebuilds the index and retries, up to three
times.
...
- **The workflow did not start**: no configured path changed, Actions are
  disabled, or the event did not target `master` as required for PR runs.
